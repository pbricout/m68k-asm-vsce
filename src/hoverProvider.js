...
exports.M68kHoverProvider = void 0;
var vscode = require("vscode");
var path = require("path");
var regexPatterns_1 = require("./regexPatterns");
var fileParser_1 = require("./fileParser");
var M68kHoverProvider = /** @class */ (function () {
    function M68kHoverProvider() {
        var _this = this;
        // Timing data tables
        this.instructionTimings = {
            'move': [
                // Long word
        ...
        this.specialCases = {
            'movem': function (context) {
                var registerCount = _this.countRegisters(context.operands[0]);
                if (context.operands[1].includes('-(')) {
                    // Memory to registers
                    return {
                        cycles: "12+4n (n=".concat(registerCount, ")"),
                        readWrite: "(3+".concat(registerCount, "/0)")
                    };
                }
                else {
                    // Registers to memory
                    return {
                        cycles: "8+4n (n=".concat(registerCount, ")"),
                        readWrite: "(2/".concat(registerCount, ")")
                    };
                }
            },
            'dbf': function () { return ({
    ...
    M68kHoverProvider.prototype.parseInstructionSize = function (line) {
        var sizeMatch = line.match(/\.(b|w|l)\b/i);
        return sizeMatch ? sizeMatch[1].toLowerCase() : null;
    };
    M68kHoverProvider.prototype.parseAddressingMode = function (operand) {
        // Enhanced addressing mode detection
        operand = operand.trim().toLowerCase();
        if (operand.match(/^d[0-7]$/))
            return 'register_direct';
        if (operand.match(/^a[0-7]$/))
            return 'address_register';
        if (operand.match(/^\(a[0-7]\)$/))
            return 'address_indirect';
        if (operand.match(/^-?\d+\(a[0-7]\)$/))
            return 'address_displacement';
        if (operand.match(/^\(a[0-7]\)\+$/))
            return 'address_postincrement';
        if (operand.match(/^-\(a[0-7]\)$/))
            return 'address_predecrement';
        if (operand.match(/^#/))
            return 'immediate';
        if (operand.match(/^\(pc\)/))
            return 'pc_relative';
        if (operand.match(/^\$[0-9a-f]{1,4}(\.[wl])?$/))
            return 'absolute_short';
        if (operand.match(/^\$[0-9a-f]{5,8}$/))
            return 'absolute_long';
        return 'other';
    };
    M68kHoverProvider.prototype.countRegisters = function (registerList) {
        // Count registers in MOVEM register list
        var matches = registerList.match(/d[0-7]|a[0-7]/gi);
        return matches ? matches.length : 0;
    };
    M68kHoverProvider.prototype.calculateTiming = function (instruction, context) {
        var _this = this;
        ...
        var timings = this.instructionTimings[instruction];
        if (!timings)
            return undefined;
        // Find matching timing entry
        var matchingEntry = timings.find(function (entry) {
            if (entry.size && entry.size !== context.size)
                return false;
            if (entry.src && entry.src !== _this.parseAddressingMode(context.operands[0]))
                return false;
            if (entry.dest && entry.dest !== _this.parseAddressingMode(context.operands[1]))
                return false;
            return true;
        });
        if (!matchingEntry)
        ...
        var _a = matchingEntry.cycles.split('-').map(Number), minCycles = _a[0], maxCycles = _a[1];
        var cycles = maxCycles ?
            "".concat(minCycles + additionalCycles, "-").concat(maxCycles + additionalCycles) :
    ...
    M68kHoverProvider.prototype.provideHover = function (document, position, token) {
        var wordRange = document.getWordRangeAtPosition(position);
        if (!wordRange)
            return null;
        var word = document.getText(wordRange).toLowerCase();
        var context = this.getInstructionContext(document, position);
        // Get base documentation
        if (this.instructionDocs[word]) {
            var documentation = this.instructionDocs[word];
            // Add context-specific timing if available
            if (context) {
                var timing = this.calculateTiming(word, context);
                if (timing) {
                    documentation += "\n\nCurrent instruction timing:\n" +
                        "* Cycles: ".concat(timing.cycles, "\n") +
                        "* Read/Write: ".concat(timing.readWrite);
                }
            }
            return new vscode.Hover(new vscode.MarkdownString(documentation), wordRange);
        }
        // Handle registers and directives
        if (this.registerDocs[word]) {
            return new vscode.Hover(new vscode.MarkdownString("**".concat(word.toUpperCase(), "**\n\n").concat(this.registerDocs[word])), wordRange);
        }
        if (this.directiveDocs[word]) {
            return new vscode.Hover(new vscode.MarkdownString("**".concat(word.toUpperCase(), "**\n\n").concat(this.directiveDocs[word])), wordRange);
        }
        // Check for user-defined symbols
        var symbolInfo = this.getSymbolInfo(document, word);
        if (symbolInfo) {
            return new vscode.Hover(new vscode.MarkdownString(symbolInfo), wordRange);
        }
        return null;
    };
    M68kHoverProvider.prototype.getSymbolInfo = function (document, symbolName) {
        try {
            var context = fileParser_1.M68kFileParser.createParseContext(document);
            var definition = fileParser_1.M68kFileParser.findSymbolDefinition(context.filePath, symbolName, context);
            if (!definition) {
                return null;
            }
            var fileName = path.basename(definition.filePath);
            var lineNumber = definition.line + 1;
            // Determine symbol type and format display
            if (definition.value !== undefined) {
                // EQU definition
                return "**".concat(symbolName, "** (Constant)\n\nValue: `").concat(definition.value, "`\nDefined at line ").concat(lineNumber, " in ").concat(fileName);
            }
            else {
                // Label definition
                return "**".concat(symbolName, "** (Label)\n\nDefined at line ").concat(lineNumber, " in ").concat(fileName);
            }
        }
        catch (error) {
    ...
    M68kHoverProvider.prototype.getInstructionDocs = function (instruction) {
        return this.instructionDocs[instruction.toLowerCase()] || '';
    };
    M68kHoverProvider.prototype.getInstructionContext = function (document, position) {
        var line = document.lineAt(position.line).text;
        var word = document.getText(document.getWordRangeAtPosition(position));
        // Match instruction and its operands using shared regex patterns
        var instructionMatch = line.match(new RegExp("\\b".concat(regexPatterns_1.M68kRegexPatterns.escapeSymbol(word), "(?:\\.(b|w|l))?\\s+([^;]+)"), 'i'));
        if (!instructionMatch)
            return null;
        var size = this.parseInstructionSize(line);
        var operandsPart = instructionMatch[instructionMatch.length - 1];
        var operands = operandsPart.split(',').map(function (op) { return op.trim(); });
        return {
    ...
    return M68kHoverProvider;
}());
exports.M68kHoverProvider = M68kHoverProvider;
